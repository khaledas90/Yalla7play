...
import { getCatalogData, getAdsByPlacement, shouldRenderInlineAd } from "@/lib/platform";
...
export const metadata: Metadata = {
  title: "الألعاب",
  description: "تصفح الألعاب حسب القسم مع ترتيب ذكي.",
};
...
export default async function GamesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const ads = await getAdsByPlacement("BETWEEN_ITEMS");
  const totalPages = Math.max(1, Math.ceil(total / pageSize));

  return (
    <main className="min-h-screen bg-transparent px-4 pb-16 pt-28 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <h1 className="text-3xl font-black text-slate-900">الألعاب</h1>

        <div className="flex flex-wrap gap-2">
          <Link href="/games" className="rounded-full border border-blue-200 bg-white px-3 py-1 text-sm text-slate-800">
            الكل
          </Link>
          {categories.map((c) => (
            <Link
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item, index) => (
            <div key={item.id}>
              <ContentCard item={item} type="games" />
              {ads[0] && shouldRenderInlineAd(index, ads[0].frequency) ? (
                <div className="mt-4">
                  <AdSlot ad={ads[0]} />
                </div>
              ) : null}
        ...
        <div className="flex items-center justify-center gap-2">
          <Link
            href={`/games?page=${Math.max(1, page - 1)}`}
          ...
          <Link
            href={`/games?page=${Math.min(totalPages, page + 1)}`}
            className="rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm text-slate-800"
          >
            التالي
          </Link>
