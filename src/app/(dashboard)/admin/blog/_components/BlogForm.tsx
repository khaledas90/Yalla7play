...
export function BlogForm({
  ...
  const targetPostId = postId || post?.id || null;
  const isEditMode = Boolean(targetPostId);
  ...
  const fetchPostData = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/blog/${targetPostId}`);
      if (!response.ok) {
        const raw = await response.text();
        let message = "فشل تحميل بيانات المقال";
        try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const tagsArray = formData.tags
        ...
        : null;
      const url = isEditMode ? `/api/admin/blog/${targetPostId}` : "/api/admin/blog";
      ...
      toast.success(isEditMode ? "تم تحديث المقال بنجاح" : "تم إضافة المقال بنجاح");
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="space-y-4 py-4 overflow-y-auto flex-1">
            ...
            <div>
              ...
              <TextEditor
                ...
                onChange={(_, htmlValue) =>
                  setFormData({ ...formData, excerpt: htmlValue })
                }
                dir="rtl"
                maxLength={500}
              />
            ...
            <div className="border-t pt-4">
              <h3 className="mb-2 font-semibold">إعدادات SEO</h3>
          ...
          <div className="flex items-center justify-between pt-4">
            ...
            <Button type="submit" disabled={isLoading || isLoadingData}>
              {isLoading
                ? "جاري الحفظ..."
                : submitLabel || (isEditMode ? "حفظ" : "إضافة")}
            </Button>
          </div>
        </form>
      )}
